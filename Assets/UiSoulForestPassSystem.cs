...
public class UiSoulForestPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiSoulForestPassCell uiPassCellPrefab;

    [SerializeField]
    private Transform cellParent;


    private List<UiSoulForestPassCell> uiPassCellContainer = new List<UiSoulForestPassCell>();
    ...
    {
        var tableData = TableManager.Instance.SoulForestPass.dataArray;

        for (int i = 0; i < tableData.Length; i++)
        {
            var prefab = Instantiate<UiSoulForestPassCell>(uiPassCellPrefab, cellParent);
        ...
        for (int i = 0; i < uiPassCellContainer.Count; i++)
        {
            if (i < tableData.Length)
            {
                ...
                passInfo.rewardType_Free_Key = ColdSeasonPassServerTable.SoulForestFree;

                passInfo.rewardType_IAP = tableData[i].Reward2;
                passInfo.rewardTypeValue_IAP = tableData[i].Reward2_Value;
                passInfo.rewardType_IAP_Key = ColdSeasonPassServerTable.SoulForestdAd;
    ...
    {
        splitData_Free = GetSplitData(ColdSeasonPassServerTable.SoulForestFree);
        splitData_Ad = GetSplitData(ColdSeasonPassServerTable.SoulForestdAd);
        ...
        string free = ServerData.coldSeasonPassServerTable.TableDatas[ColdSeasonPassServerTable.SoulForestFree].Value;
        string ad = ServerData.coldSeasonPassServerTable.TableDatas[ColdSeasonPassServerTable.SoulForestdAd].Value;
        ...
        if (rewardedNum > 0)
        {
            ServerData.coldSeasonPassServerTable.TableDatas[ColdSeasonPassServerTable.SoulForestFree].Value = free;
            ServerData.coldSeasonPassServerTable.TableDatas[ColdSeasonPassServerTable.SoulForestdAd].Value = ad;
            ...
            passParam.Add(ColdSeasonPassServerTable.SoulForestFree, ServerData.coldSeasonPassServerTable.TableDatas[ColdSeasonPassServerTable.SoulForestFree].Value);
            passParam.Add(ColdSeasonPassServerTable.SoulForestdAd, ServerData.coldSeasonPassServerTable.TableDatas[ColdSeasonPassServerTable.SoulForestdAd].Value);
            ...
            ServerData.SendTransaction(transactions, successCallBack: () =>
            {
                ...
                //LogManager.Instance.SendLogType("SoulForestPass", "A", "A");
            });
        }
        else
        {
    ...
    {
        var relicScore = ServerData.userInfoTable.GetTableData(UserInfoTable.relicKillCount).Value;
        return relicScore >= require;
    }
    ...
    {
        bool hasIapProduct = ServerData.iapServerTable.TableDatas[UiSoulForestPassBuyButton.seasonPassKey].buyCount.Value > 0;
