...
public class YeonOkTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.yeonokTowerIdx).Value >=
                                               TableManager.Instance.YeonokTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.yeonokTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.YeonokTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/YeonokTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.yeonokTowerIdx).Value;

        var tableData = TableManager.Instance.YeonokTowerTable.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.YeonokTowerTable.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.yeonokTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable_2.yeonokTowerIdx, ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.yeonokTowerIdx).Value);
