...
public class MunhaTower2Manager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaTower).Value >=
                                               TableManager.Instance.StudentTower.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaTower).Value;
    ...
    {
        stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaTower).Value;

        var tableData = TableManager.Instance.StudentTower.dataArray[stageId];

        EnemyTableData enemy = new EnemyTableData();

        enemy.Prefabname = "MunhaTower2";
        enemy.Movespeed = 0;
    ...
    {
        //단계상승
        ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaTower).Value++;

        List<TransactionValue> transactionList = new List<TransactionValue>();

        Param floorParam = new Param();

        var score = ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaTower).Value;

        floorParam.Add(UserInfoTable_2.munhaTower, score);
