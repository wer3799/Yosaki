...
public class UiHaetalPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiHaetalPassCell uiPassCellPrefab;
    ...
    private List<UiHaetalPassCell> uiPassCellContainer = new List<UiHaetalPassCell>();
    ...
    private void Start()
    {
        ...
        var level = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.haetalGradeIdx).Value;
    ...
    {
        var tableData = TableManager.Instance.HaetalPass.dataArray;
        for (int i = 0; i < tableData.Length; i++)
        {
            var prefab = Instantiate<UiHaetalPassCell>(uiPassCellPrefab, cellParent);
        ...
        for (int i = 0; i < uiPassCellContainer.Count; i++)
        {
            if (i < tableData.Length)
            {
                ...
                passInfo.rewardType_Free_Key = ColdSeasonPassServerTable.haetalFree;

                passInfo.rewardType_IAP = tableData[i].Reward2;
                passInfo.rewardTypeValue_IAP = tableData[i].Reward2_Value;
                passInfo.rewardType_IAP_Key = ColdSeasonPassServerTable.haetalAd;
    ...
    public void OnClickAllReceiveButton()
    {
        ...
        string freeKey = ColdSeasonPassServerTable.haetalFree;
        string adKey = ColdSeasonPassServerTable.haetalAd;
    ...
    {
        return ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.haetalGradeIdx).Value >= require;
    }
    ...
    {
        return ServerData.iapServerTable.TableDatas[PassBuyButton.haetalPassKey].buyCount.Value > 0;
    }
}
