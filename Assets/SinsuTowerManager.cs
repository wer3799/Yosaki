...
public class SinsuTowerManager : ContentsManagerBase
{
    ...
    {
        //int currentScore = (int)DatabaseManager.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx6).Value;
    ...
    {
        int stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx6).Value;

        var TowerTableData2 = TableManager.Instance.sinsuTower.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/SinsuTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx6).Value;

        var tableData = TableManager.Instance.sinsuTower.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.sinsuTower.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx6).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable.currentFloorIdx6, ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx6).Value);
