...
public class TransTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.transTowerIdx).Value >=
                                               TableManager.Instance.TransTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.transTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.TransTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < 1; i++)
        {
            poolName = $"Enemy/TransTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.transTowerIdx).Value;

        var tableData = TableManager.Instance.TransTowerTable.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.TransTowerTable.dataArray[currentFloor];
...
//
        ...
        //단계상승
        ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.transTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable_2.transTowerIdx, ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.transTowerIdx).Value);
