...
public class UiDosulLevelPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiDosulLevelPassCell uiPassCellPrefab;
    ...
    private List<UiDosulLevelPassCell> uiPassCellContainer = new List<UiDosulLevelPassCell>();
    ...
    private void Start()
    {
        ...
        var str = ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.dosulLevel).Value < 0 ? "도술 레벨 : 없음" : $"도술 레벨 : {ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.dosulLevel).Value+1}";
    ...
    {
        var tableData = TableManager.Instance.DosulLevelPass.dataArray;

        for (int i = 0; i < tableData.Length; i++)
        {
            var prefab = Instantiate<UiDosulLevelPassCell>(uiPassCellPrefab, cellParent);
        ...
        for (int i = 0; i < uiPassCellContainer.Count; i++)
        {
            if (i < tableData.Length)
            {
                ...
                passInfo.rewardType_Free_Key = ColdSeasonPassServerTable.dosulLevelFree;

                passInfo.rewardType_IAP = tableData[i].Reward2;
                passInfo.rewardTypeValue_IAP = tableData[i].Reward2_Value;
                passInfo.rewardType_IAP_Key = ColdSeasonPassServerTable.dosulLevelAd;
    ...
    public void OnClickAllReceiveButton()
    {
        ...
        string freeKey = ColdSeasonPassServerTable.dosulLevelFree;
        string adKey = ColdSeasonPassServerTable.dosulLevelAd;
