...
public class SealSwordTowerManager : ContentsManagerBase
{
   [SerializeField]
    ...
    {
        //int currentScore = (int)DatabaseManager.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx9).Value;
    ...
    {
        int stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx9).Value;

        var TowerTableData2 = TableManager.Instance.SealTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < 1; i++)
        {
            poolName = $"Enemy/SealSword/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx9).Value;

        var tableData = TableManager.Instance.SealTowerTable.dataArray[stageId];

        EnemyTableData enemy = new EnemyTableData();

        enemy.Prefabname = tableData.Materialtype;
        enemy.Attackpower =0f;
        enemy.Movespeed = 0f;
        enemy.Hp = tableData.Hp;
        enemy.Knockbackpower = 0f;
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.SealTowerTable.dataArray[currentFloor];
...
//
        ...
        //단계상승
        ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx9).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable.currentFloorIdx9, ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx9).Value);
