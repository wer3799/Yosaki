...
public class DarkTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable.GetTableData(UserInfoTable.DarkTowerIdx).Value >=
                                               TableManager.Instance.DarkTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.DarkTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.DarkTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/DarkTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.DarkTowerIdx).Value;

        var tableData = TableManager.Instance.DarkTowerTable.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.DarkTowerTable.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable.GetTableData(UserInfoTable.DarkTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable.DarkTowerIdx, ServerData.userInfoTable.GetTableData(UserInfoTable.DarkTowerIdx).Value);
