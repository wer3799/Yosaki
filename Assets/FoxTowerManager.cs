...
public class FoxTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    {
        //int currentScore = (int)DatabaseManager.userInfoTable.GetTableData(UserInfoTable.foxTowerIdx).Value;
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable.GetTableData(UserInfoTable.foxTowerIdx).Value >=
                                               TableManager.Instance.FoxTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.foxTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.FoxTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < 1; i++)
        {
            poolName = $"Enemy/FoxTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.foxTowerIdx).Value;

        var tableData = TableManager.Instance.FoxTowerTable.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.FoxTowerTable.dataArray[currentFloor];
...
//
        ...
        //단계상승
        ServerData.userInfoTable.GetTableData(UserInfoTable.foxTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable.foxTowerIdx, ServerData.userInfoTable.GetTableData(UserInfoTable.foxTowerIdx).Value);
