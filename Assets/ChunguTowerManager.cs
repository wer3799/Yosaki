...
public class ChunguTowerManager : ContentsManagerBase
{
    ...
    {
        //int currentScore = (int)DatabaseManager.userInfoTable.GetTableData(UserInfoTable_2.chunguTowerIdx).Value;
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            if (ServerData.userInfoTable_2.IsLastFloor4() == false)
            {
    ...
    {
        int stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunguTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.TowerTable16.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/ChunguTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunguTowerIdx).Value;

        var tableData = TableManager.Instance.TowerTable16.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.TowerTable16.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunguTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable_2.chunguTowerIdx, ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunguTowerIdx).Value);
