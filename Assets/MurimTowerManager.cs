...
public class MurimTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.MurimTowerIdx).Value >=
                                               TableManager.Instance.MurimTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.MurimTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.MurimTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/MurimTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.MurimTowerIdx).Value;

        var tableData = TableManager.Instance.MurimTowerTable.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.MurimTowerTable.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.MurimTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable_2.MurimTowerIdx, ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.MurimTowerIdx).Value);
