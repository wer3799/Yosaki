...
public class UiSAShopCell : MonoBehaviour
{
    [SerializeField]
    ...
    public void OnClickExchangeButton()
    {
        ...
        int currentEventItemNum = (int)ServerData.goodsTable.GetTableData(GoodsTable.Event_SA).Value;

        if (currentEventItemNum < tableData.Price)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.Event_SA)}가 부족합니다.");
            return;
        }
        ...
        //로컬
        ServerData.goodsTable.GetTableData(GoodsTable.Event_SA).Value -= tableData.Price;
