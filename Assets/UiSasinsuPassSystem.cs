...
public class UiSasinsuPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiSasinsuPassCell uiPassCellPrefab;
    ...
    private List<UiSasinsuPassCell> uiPassCellContainer = new List<UiSasinsuPassCell>();
    ...
    private void Start()
    {
        ...
        var str =  level <0 ? "사신수 각성 단계 : 0" : $"사신수 각성 단계 : {level}단계";
    ...
    {
        var tableData = TableManager.Instance.PetAwakePass.dataArray;

        for (int i = 0; i < tableData.Length; i++)
        {
            var prefab = Instantiate<UiSasinsuPassCell>(uiPassCellPrefab, cellParent);
        ...
        for (int i = 0; i < uiPassCellContainer.Count; i++)
        {
            if (i < tableData.Length)
            {
                ...
                passInfo.rewardType_Free_Key = ColdSeasonPassServerTable.sasinsuFree;

                passInfo.rewardType_IAP = tableData[i].Reward2;
                passInfo.rewardTypeValue_IAP = tableData[i].Reward2_Value;
                passInfo.rewardType_IAP_Key = ColdSeasonPassServerTable.sasinsuAd;
    ...
    public void OnClickAllReceiveButton()
    {
        ...
        string freeKey = ColdSeasonPassServerTable.sasinsuFree;
        string adKey = ColdSeasonPassServerTable.sasinsuAd;
    ...
    public bool CanGetReward(int require)
    {
        ...
        return ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.sasinsuAwakeGrade).Value >= require;
    }
    ...
    {
        return ServerData.iapServerTable.TableDatas[PassBuyButton.sasinsuPassKey].buyCount.Value > 0;
    }
}
