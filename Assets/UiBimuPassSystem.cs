...
public class UiBimuPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiBimuPassCell uiPassCellPrefab;
    ...
    private List<UiBimuPassCell> uiPassCellContainer = new List<UiBimuPassCell>();
    ...
    private void Start()
    {
        ...
        var level = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.battleContestGradeLevel).Value;
        
        var str =  level <0 ? "등급 없음" : $"최고 등급 : {TableManager.Instance.BattleContestGrade.dataArray[level].Title_Name}";
    ...
    {
        var tableData = TableManager.Instance.BimuPass.dataArray;

        for (int i = 0; i < tableData.Length; i++)
        {
            var prefab = Instantiate<UiBimuPassCell>(uiPassCellPrefab, cellParent);
        ...
        for (int i = 0; i < uiPassCellContainer.Count; i++)
        {
            if (i < tableData.Length)
            {
                ...
                passInfo.rewardType_Free_Key = ColdSeasonPassServerTable.bimuFree;

                passInfo.rewardType_IAP = tableData[i].Reward2;
                passInfo.rewardTypeValue_IAP = tableData[i].Reward2_Value;
                passInfo.rewardType_IAP_Key = ColdSeasonPassServerTable.bimuAd;
    ...
    public void OnClickAllReceiveButton()
    {
        ...
        string freeKey = ColdSeasonPassServerTable.bimuFree;
        string adKey = ColdSeasonPassServerTable.bimuAd;
    ...
    {
        //return ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.dosulLevel).Value >= require;
        return ServerData.goodsTable.GetTableData(GoodsTable.BattleScore).Value >= require;
    }
