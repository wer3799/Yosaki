...
public class UiStudentPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiStudentPassCell uiPassCellPrefab;
    ...
    private List<UiStudentPassCell> uiPassCellContainer = new List<UiStudentPassCell>();
    ...
    private void Start()
    {
        ...
        var level = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaLevel).Value;
        
        var str =  level <0 ? "제자 단계 : 0" : $"제자 단계 : {level+1}단계";
    ...
    {
        var tableData = TableManager.Instance.StudentPass.dataArray;

        for (int i = 0; i < tableData.Length; i++)
        {
            var prefab = Instantiate<UiStudentPassCell>(uiPassCellPrefab, cellParent);
        ...
        for (int i = 0; i < uiPassCellContainer.Count; i++)
        {
            if (i < tableData.Length)
            {
                ...
                passInfo.rewardType_Free_Key = ColdSeasonPassServerTable.studentFree;

                passInfo.rewardType_IAP = tableData[i].Reward2;
                passInfo.rewardTypeValue_IAP = tableData[i].Reward2_Value;
                passInfo.rewardType_IAP_Key = ColdSeasonPassServerTable.studentAd;
    ...
    public void OnClickAllReceiveButton()
    {
        ...
        string freeKey = ColdSeasonPassServerTable.studentFree;
        string adKey = ColdSeasonPassServerTable.studentAd;
    ...
    {
        return ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.munhaLevel).Value >= require;
    }
    ...
    {
        return ServerData.iapServerTable.TableDatas[PassBuyButton.studentPassKey].buyCount.Value > 0;
    }
}
