...
public class UiGyungRockMassgeBoard7 : MonoBehaviour
{
 [SerializeField] private TextMeshProUGUI currentFloorDescription;
    ...
    {
        totalAbil.SetText(
            ($"{CommonString.GetStatusName((StatusType.SuperCritical33DamPer))} 총 {PlayerStats.GetGyungRockEffect7(StatusType.SuperCritical33DamPer) * 100f}% 적용됨"));
        ...
        currentFloorAbil.SetText($"혈자리(극혈) {currentFloor}개 개방됨");
        ...
        dokChimEffect.SetText($"전갈굴(문파) 독침 효과로 극혈베기 피해량 {Utils.ConvertBigNumForRewardCell(Mathf.Round(PlayerStats.GetGuildTowerChimUpgradeValue() * 100f))}% 증폭됨<color=yellow>(+{PlayerStats.GetGuildTowerChimUpgradeValue() * PlayerStats.GetGyungRockEffect7(StatusType.SuperCritical33DamPer) * 100f}%)</color>");
    ...
    public void OnClickEnterButton()
    {
        ...
        PopupManager.Instance.ShowYesNoPopup(CommonString.Notice, "입장 하시겠습니까?", () => { GameManager.Instance.LoadContents(GameManager.ContentsType.GyungRockTower7); }, () => { });
    }
}
