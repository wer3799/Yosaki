...
public class UiMonthPassSystem : FancyScrollView<MonthlyPassData_Fancy>
{
    ...
    public void OnClickAllReceiveButton()
    {
        ...
        var passValue = int.Parse(ServerData.monthlyPassServerTable
            .TableDatas[MonthlyPassServerTable.MonthlypassFreeReward].Value) + 1;
        var adValue = int.Parse(ServerData.monthlyPassServerTable
            .TableDatas[MonthlyPassServerTable.MonthlypassAdReward].Value) + 1;
        ...
        string free = ServerData.monthlyPassServerTable.TableDatas[MonthlyPassServerTable.MonthlypassFreeReward]
            .Value;
        ...
        for (int i = passValue; i < tableData.Length; i++)
        {
            ...
            //받은적 있는지 체크
            if (HasReward(MonthlyPassServerTable.MonthlypassFreeReward, tableData[i].Id) == false)
            {
                //코스튬이나 노리개가 있다면?
