...
public class DimensionStatusTypeDictionary : EditorWindow
{
  [SerializeField] private int m_SelectedIndex = -1;
  private VisualElement secondPane;
  [MenuItem("Tools/DimensionStatusTypeDictionary")]
  public static void ShowMyEditor()
  {
    // This method is called when the user selects the menu item in the Editor
    EditorWindow wnd = GetWindow(typeof(DimensionStatusTypeDictionary)) as DimensionStatusTypeDictionary;
    wnd.titleContent = new GUIContent("DimensionStatusTypeDictionary");
  ...
  public void CreateGUI()
  {
    ...
    DimensionStatusType[] values = GetValues<DimensionStatusType>();
    firstPane.makeItem = () => new Label();
    firstPane.bindItem = (item, index) => { 
      int value = (int)values[index];
      string name = values[index].ToString();
      string name2 = CommonString.GetStatusName((DimensionStatusType)value);
      (item as Label).text =
