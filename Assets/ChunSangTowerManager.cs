...
public class ChunSangTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunsangTowerIdx).Value >=
                                               TableManager.Instance.SkyTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunsangTowerIdx).Value;

        var TowerTableData2 = TableManager.Instance.SkyTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/ChunSangTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunsangTowerIdx).Value;

        var tableData = TableManager.Instance.SkyTowerTable.dataArray[stageId];
    ...
    {
        //보상지급
        int currentFloor = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunsangTowerIdx).Value;

        var TowerTableData3 = TableManager.Instance.SkyTowerTable.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunsangTowerIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable_2.chunsangTowerIdx, ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.chunsangTowerIdx).Value);
