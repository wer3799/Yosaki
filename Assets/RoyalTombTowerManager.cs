...
public class RoyalTombTowerManager : ContentsManagerBase
{
    [SerializeField]
    ...
    private IEnumerator StartAutoStart()
    {
        ...
        {
            GameManager.Instance.LoadContents((ServerData.userInfoTable_2.GetTableData(UserInfoTable.RoyalTombFloorIdx).Value >=
                                               TableManager.Instance.royalTombTowerTable.dataArray.Length) == false
    ...
    {
        int stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.RoyalTombFloorIdx).Value;

        var TowerTableData2 = TableManager.Instance.royalTombTowerTable.dataArray[stageId];
        EnemyTableData spawnEnemyData = GetSpawnedEnemy(stageId);

        for (int i = 0; i < TowerTableData2.Spawnnum; i++)
        {
            poolName = $"Enemy/RoyalTombTower/{spawnEnemyData.Prefabname}";
    ...
    {
        stageId = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.RoyalTombFloorIdx).Value;

        var tableData = TableManager.Instance.royalTombTowerTable.dataArray[stageId];
    ...
    private void SetClear()
    {
        ...
        var TowerTableData3 = TableManager.Instance.royalTombTowerTable.dataArray[currentFloor];
        ...
        //단계상승
        ServerData.userInfoTable.GetTableData(UserInfoTable.RoyalTombFloorIdx).Value++;

        Param floorParam = new Param();

        floorParam.Add(UserInfoTable.RoyalTombFloorIdx, ServerData.userInfoTable.GetTableData(UserInfoTable.RoyalTombFloorIdx).Value);
