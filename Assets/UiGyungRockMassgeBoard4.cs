...
public class UiGyungRockMassgeBoard4 : MonoBehaviour
{
 [SerializeField] private TextMeshProUGUI currentFloorDescription;
    ...
    {
        totalAbil.SetText(
            ($"{CommonString.GetStatusName((StatusType.SuperCritical8DamPer))} 총 {PlayerStats.GetGyungRockEffect4(StatusType.SuperCritical8DamPer) * 100f}% 적용됨"));

        int currentFloor = (int)ServerData.userInfoTable_2.GetTableData(UserInfoTable_2.gyungRockTower4).Value;

        currentFloorDescription.SetText($"{currentFloor}단계");

        currentFloorAbil.SetText($"혈자리(하단전) {currentFloor}개 개방됨");

        if (currentFloor >= TableManager.Instance.gyungRockTowerTable4.dataArray.Length)
        {
        ...
        dokChimEffect.SetText($"전갈굴(문파) 독침 효과로 하단전베기 피해량 {Utils.ConvertBigNumForRewardCell(Mathf.Round(PlayerStats.GetGuildTowerChimUpgradeValue() * 100f))}% 증폭됨<color=yellow>(+{PlayerStats.GetGuildTowerChimUpgradeValue() * PlayerStats.GetGyungRockEffect4(StatusType.SuperCritical8DamPer) * 100f}%)</color>");
    ...
    public void OnClickEnterButton()
    {
        ...
        if (currentFloor < 0 || currentFloor >= TableManager.Instance.gyungRockTowerTable4.dataArray.Length)
        {
            PopupManager.Instance.ShowAlarmMessage(("최고 단계 입니다!"));
            return;
        }

        PopupManager.Instance.ShowYesNoPopup(CommonString.Notice, "입장 하시겠습니까?", () => { GameManager.Instance.LoadContents(GameManager.ContentsType.GyungRockTower4); }, () => { });
    }
}
