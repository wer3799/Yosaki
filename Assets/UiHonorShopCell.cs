...
public class UiHonorShopCell : MonoBehaviour
{
    [SerializeField]
    ...
    private GuildExchangeData tableData;
    ...
    private void Initialize()
    {
        ...
        tableData = TableManager.Instance.GuildExchange.dataArray[tableId];
    ...
    public void OnClickExchangeButton()
    {
        ...
        int currentEventItemNum = (int)ServerData.goodsTable.GetTableData(GoodsTable.HonorGoods).Value;

        if (currentEventItemNum < tableData.Price)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.HonorGoods)}가 부족합니다.");
            return;
        }
        ...
        //로컬
        ServerData.goodsTable.GetTableData(GoodsTable.HonorGoods).Value -= tableData.Price;
    ...
    public void OnClickExchangeAllButton()
    {
        PopupManager.Instance.ShowYesNoPopup(CommonString.Notice, "전부 교환 합니까?", () =>
        {
            ...
            //로컬
            ServerData.goodsTable.GetTableData(GoodsTable.HonorGoods).Value -= tableData.Price * exchangeCount;
